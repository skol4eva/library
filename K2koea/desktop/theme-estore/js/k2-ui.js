		...
		fn_prodDetail: function(options){
			...
			return this.each(function(){
				var $this = $(this),
					...
					_end = $this.offset().top+$this.outerHeight()-_win.height();
				...
				function btnOffset(){
					if(_win.scrollTop()>=_start){
						if(_win.scrollTop()>=_end){
							_btn.finish().removeClass('fixed').css({'bottom':'-50px'});
							_btn.prev().css({'margin-bottom':0});
						}else{
							_btn.addClass('fixed').delay(100).animate({'bottom':0});
							_btn.prev().css({'margin-bottom':_btn.height()+90});
						}
					}else{
						_btn.finish().removeClass('fixed').css({'bottom':'-50px'})
...
function toggleList(){
	...
	if($toggle.hasClass('mp_review_list')){ // 마이페이지 :: 상품리뷰 & 상품문의 예외처리
