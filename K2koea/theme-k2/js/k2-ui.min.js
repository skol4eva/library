$(function(){
	$.fn.extend({
		...
		fn_orderFixed : function(options){
			var $header = $('.ui-header'),
				$headerHei = $('.ui-header').height(),
				$container = $('.ui-container'),
				$containerHei = $('.ui-container').height(),
				$orderPym = $('.payment_area');

			$(window).bind('scroll', function(){
				if($(window).scrollTop() >= $headerHei){
					$orderPym.addClass('fixed');
					$orderPym.css({'position': 'fixed', 'top': ($headerHei / 2) - 60});
				}else{
					$orderPym.removeClass('fixed');
					$orderPym.css({'position':'relative','top':'0'});
				}
			});
		},//fn_orderFixed

		//fn_ord_tabs
		fn_ord_tabs: function(options){
			var defaults = {};
			var opts = $.extend(defaults, options);
			return this.each(function(){
				var $this = $(this),
					$tabList = $this.children('.chkRd_box'),
					$panel = $('.payment_info').find('.tabs-panel').children('.panel-section'),
					$current = $this.parent('.current').index();
	...
	// oder payment
	// $('.payment_area').fn_orderFixed();

	// fn_ord_tabs
	$('.pym_tab > li').fn_ord_tabs();
...
$(function(){
	...
	//sky
	var sky = $('.payment_area');

	if (sky.length) {
		var doc = $(window),
			foot_t = $('.ui-footer').offset().top;

		sky.attr('data-top', 190).addClass('fixed');
...
//order escrow check
	var $pymChk = $('.pym_tab .chkRd_box');
	var $escrowChk = $('.pym_tab .chk_escrow');
	var $escrowCnts = $('.pym_cnts .cnts_section .escrow');
	var $depositChk = $escrowChk.prev('.chkRd_box');

	$pymChk.on('click', function(){
		$escrowChk.find('.styledCheckbox')
			.attr('aria-checked','flase')
			.css('background-position','0 0');
	});

	$escrowChk.on('click', function () {
		$escrowCnts.toggleClass('on');
		$depositChk.find('.styledRadio')
