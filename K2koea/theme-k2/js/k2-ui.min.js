	...
	$.fn.extend({
		...
		fn_order_fixed : function(options){
			...
			var doc = $(window),
				...
				foot_h = $('.ui-footer').height(),
				sky = $(this),
				leftCnt = $('.order_info_area').outerWidth(true)

			sky.attr('data-top', 294).addClass('fixed');
			doc.on('scroll', function() {
				var sc_t = doc.scrollTop();

				var _this = sky,//ul.payment
					_top = _this.data('top'),//header
					_h = _this.height(),//ul.payment height
					_css_t = (_top - sc_t),//header - document current positon
					_css_t = _css_t <= 0 ? 10 : _css_t,
					_lap = (foot_t - sc_t),//footer position - document current positon
					_this_end = (_h - _lap);
		...
		fn_ord_tabs: function(options){
			...
			return this.each(function(){
                var $this = $(this);
                var $tabList = $this.find('.chkRd_box');
                var $panel = $('.payment_info').find('.tabs-panel').children('.panel-section');
                var $current = $this.find('.current').index();
...
$(function(){
    ...
    var	$allChk = $('.all_chk').find('input:checkbox'),
		...
		$agrChkLeng = $('.agr_chk').find('input:checkbox').length;
